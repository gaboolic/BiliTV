...
- **首页 = 主题行**：只有「我的世界」「芭比娃娃」等已启用的主题，一屏一行主题，没有推荐、热门、分区。
...
| 开关 | 作用 |
|------|------|
| 儿童模式 | 总开关。关闭后恢复原来的推荐/分区首页（方便自己调试） |
| 自动信任通过的UP主 | 是否根据主题搜索结果自动学习 UP 主 |
| 我的世界 / 芭比娃娃 / 小猪佩奇 … | 逐个主题开关，内置 14 个主题，默认为「我的世界」「芭比娃娃」 |
| 信任的UP主 | 显示当前信任数量，可清空 |

主题（搜索词 + 标题匹配关键词）定义在 `lib/config/kids_topics.dart`，想加自己的主题在这里加一条 `KidsTopic` 即可。
